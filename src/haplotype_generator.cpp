...
string generated_hap(igraph_t* graph, 
    ...
    cout << "Starting on haplotype " << hap << endl;
    ...
    while(out_degree > 0 && len < target_len){
        ...
        if(possible_recombinations.size() > 0){
            ...
            if(random_float <= PROB_RECOMBINATION){
                ...
                cout << "Changing to haplotype " << hap << " at sequence position " << len << endl;
                ...
                cout << "Ending path early, no recombination taken and no way to continue on current path" << endl;
                break;
            }
        }
